...
exports.getShippedOrders = [shipped, asyncHandler(async (req, res) => {
    try {
        const orders = await Order.find({ ShippingCompany: req.user._id ,  orderStatus: 'shipped' }).populate('user', 'name email Address').populate('items.product', 'name price imageUrl Address');
